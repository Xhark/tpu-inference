    ...
    """
        ...
        for b in range(batch_size):
            ...
            def _update(b=b, offset=offset, update_sz=update_sz):
                ...
                new_kv_packing_offset = unsigned_mod(new_kv_len_start,
                                                     kv_packing)

                token_offset_in_bkv = unsigned_mod(offset, bkv_sz)
                kv_packing_idx = unsigned_floor_div(token_offset_in_bkv,
                                                    kv_packing)
                ...
                def merge_loop_body(i, vals):
                    ...
                    if num_sublanes_for_kv_packing == 1:
                        curr_kvc_reg_u32 = pltpu.bitcast(
                            curr_kvc_reg, jnp.uint32)
                        curr_kpe_reg_u32 = pltpu.bitcast(
                            curr_kpe_reg, jnp.uint32)
                        next_kvc_reg_u32 = pltpu.bitcast(
                            next_kvc_reg, jnp.uint32)
                        next_kpe_reg_u32 = pltpu.bitcast(
                            next_kpe_reg, jnp.uint32)
                        ...
                        rolled_kvc = pltpu.bitcast(rotated_kvc_u32,
                                                   next_kvc_reg.dtype)
                        rolled_kpe = pltpu.bitcast(rotated_kpe_u32,
                                                   next_kpe_reg.dtype)
                    else:
                        ...
                        rolled_kvc = lax.select(roll_amount == 0, curr_kvc_reg,
                                                rolled_kvc)
                        rolled_kpe = lax.select(roll_amount == 0, curr_kpe_reg,
                                                rolled_kpe)
