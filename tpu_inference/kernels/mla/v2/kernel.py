...
def _mla_ragged_paged_attention_kernel(
  ...
  debug_print(
      "[RPA debug] ======= In loop batch_start_seq_idx={}",
      batch_start_seq_idx)
  ...
  def batch_flash_attention(
    ...
    for b in range(batch_size):
      ...
      q_span = (
          ...
          - q_len
          + bq_idx * bq_sz
          + unsigned_floor_div(lax.broadcasted_iota(jnp.int32, s.shape[1:], 0),
                               num_q_heads)
      )
      mask = q_span < k_span
    ...
    pv = jnp.einsum(
        ...
        p, kv_c, preferred_element_type=jnp.float32
    )
    ...
    if bkv_sz % 256 == 0:
      ...
      for partial_idx in range(bkv_sz // 256):
        ...
        s_c_partial = jnp.einsum(
            "nd,dm->nm",
            ql_nope, kv_c_partial, preferred_element_type=jnp.float32)
        s_pe_partial = jnp.einsum(
            "nd,md->nm" if not transpose_kv_cache else "nd,dm->nm",
            q_pe, k_pe_partial, preferred_element_type=jnp.float32)
  ...
  def flash_attention_step2_pv(
    ...
    if p_same_dtype_as_v:
      p = p.astype(v.dtype)
    pv = jnp.einsum(
        "nm,md->nd" if not transpose_kv_cache else "nm,dm->nd",
  ...
  def _fetch_transposed_bkv(
      batch_start_seq_idx, bkv_idx, bkv_sem_idx, *, wait=False):
    if not wait:
    ...
    for b in range(batch_size):
      ...
      if not wait:
        ...
        debug_print(
            "[RPA debug] aligned_new_kv_len_start={}",
  ...
  def _pack_new_transposed_kv(bkv_sem_idx, bkv_idx, offsets, update_szs):
    ...
    for b in range(batch_size):
      ...
      def _update(b=b, update_sz=update_sz):
        ...
        loop_sz = unsigned_floor_div(
            aligned_new_kv_len_size, 128)
        ...
        def copy_partial(new_kv_idx, new_kv_start, bkv_idx, bkv_start):
          ...
          debug_print(
              ...
              new_kv_idx, new_kv_start, bkv_idx, bkv_start, new_kvc_idx,
          )
  ...
  def _update_transposed_kv_cache(
    ...
    if not wait:
      ...
      def loop_body(i, states):
        ...
        sz_words = jnp.minimum(page_size_per_lane - curr_word_in_page, words_to_transfer)
        ...
        debug_print(
            ...
            i, curr_word_in_page, words_to_transfer, curr_word_in_vmem, sz_words, page_idx
        )
  ...
  def start_update_kv_cache(start_seq_idx, bkv_sem_idx, offsets, update_szs):
    for b in range(batch_size):
      ...
      @pl.when(update_sz > 0)
      def _(
          b=b, update_sz=update_sz, start_seq_idx=start_seq_idx, offset=offset
      ):
  ...
  def load_bq(batch_item_idx, bq_sem_idx, *, actual_bq_sz=bq_sz):
    q_nope_ref = (
        ...
        .reshape(bq_sz * num_q_heads_per_q_packing, lkv_dim)
    )
    q_nope_vec = pltpu.bitcast(
        q_nope_ref[: actual_bq_sz * num_q_heads_per_q_packing],
        q_dtype,
    ...
    q_rope_ref = (
        bq_rope_x2_ref.bitcast(jnp.uint32)
        .at[bq_sem_idx, batch_item_idx]
        .reshape(bq_sz * num_q_heads_per_q_packing, r_dim)
    )
    q_rope_vec = pltpu.bitcast(
        q_rope_ref[: actual_bq_sz * num_q_heads_per_q_packing],
        q_dtype,
      ...
      def compute_with_bkv(bkv_idx, _):
        ...
        if two_step_flash_attention:
          ...
          for b in range(batch_size):
            ...
            debug_print(
                "[RPA debug] bkvc.shape={}, {}", bkvc.shape[0], bkvc.shape[1]
            )
            debug_print(
                "[RPA debug] bkpe.shape={}, {}", bkpe.shape[0], bkpe.shape[1]
            )
          ...
          debug_print(
              ...
              bkvc.shape[0], bkvc.shape[1], bkvc.shape[2],
          )
          debug_print(
              "[RPA debug] bkpe.shape={}, {}, {}",
              bkpe.shape[0], bkpe.shape[1], bkpe.shape[2],
          )
...
def prepare_q_nope_inputs(
  ...
  try:
    q = xpose_pipeline(
        q, transpose_axes=(1, 0, 2), n_tile=128, m_tile=32
    )[0]
...
def mla_ragged_paged_attention(
  ...
  q_pe = prepare_q_inputs(
      q_pe
  )  # [max_num_tokens, num_q_heads, r_dim]
  if not transpose_kv_cache:
  ...
  batch_distribution = (
      distribution[0] // decode_batch_size) * decode_batch_size
  # Batched decode
